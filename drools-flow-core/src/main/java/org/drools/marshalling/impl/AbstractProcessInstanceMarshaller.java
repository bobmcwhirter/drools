...
public abstract class AbstractProcessInstanceMarshaller implements
    ...
    public NodeInstance readNodeInstance(MarshallerReaderContext context,
        ...
        nodeInstance.setProcessInstance((org.drools.workflow.instance.WorkflowProcessInstance) processInstance);
        ...
        switch (nodeType) {
            ...
            case PersisterEnums.DYNAMIC_NODE_INSTANCE:
                int nbVariables = stream.readInt();
                if (nbVariables > 0) {
                    Context variableScope = ((org.drools.process.core.Process) ((org.drools.process.instance.ProcessInstance)
            	...
            	for (int i = 0; i < exclusiveGroupInstances; i++) {
                    ExclusiveGroupInstance exclusiveGroupInstance = new ExclusiveGroupInstance();
                    ((org.drools.process.instance.ProcessInstance) processInstance).addContextInstance(ExclusiveGroup.EXCLUSIVE_GROUP, exclusiveGroupInstance);
                    int nodeInstances = stream.readInt();
                    for (int j = 0; j < nodeInstances; j++) {
                        long nodeInstanceId = stream.readLong();
